...
// bars
var bars = {
	thickness: Math.pow(2 / 3, 3),
	map: {
		res: {
			width: Math.round(screen.width * thickness),
			height: screen.height,
			pow: powerRes(Math.max(bars.map.res.width, bars.map.res.height)),
		},
		arr: new Uint8Array(bars.map.res.pow * bars.map.res.pow * 4),
		tex: new THREE.DataTexture(bars.map.arr, bars.map.res.pow, bars.map.res.pow),
	},
	a: {
		'geo': new THREE.PlaneGeometry( /*thickness * (screen.diagonal / screen.width)*/ screen.width / screen.height * 0.5, 0.5 ), // << dup //screen.diagonal / screen.width
		'mat': new THREE.MeshBasicMaterial({ 
			map: bars.map.tex,
			color: 0xffffff,
			//wireframe: true,
		}),
		'mesh': new THREE.Mesh(bars.a.geo, bars.a.mat),
	},
}
